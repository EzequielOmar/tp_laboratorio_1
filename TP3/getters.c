...
int validateStr_Alpha(char* pResultado){
    int ret;
    int i=0;
    while(pResultado[i] != '\0'){
        if(pResultado[i] < 32 || (pResultado[i] > 32 && pResultado[i] < 45) ||
          (pResultado[i] > 45 && pResultado[i] < 65) || (pResultado[i] > 90 && pResultado[i] < 97)
          || pResultado[i] > 122){
            break;
        }
        i++;
